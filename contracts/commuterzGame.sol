...
contract CommuterzGame {
    ...
    function getNumOfTickets() constant returns(uint) {
        return tickets.length; 
    }
    
    function getUserData( uint ticketNumber ) constant returns(address,uint,uint) {
        return (tickets[ticketNumber].user,
                tickets[ticketNumber].amount,
                tickets[ticketNumber].sumTicketsAmounts);                
    }
}
