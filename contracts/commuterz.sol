...
contract Commuterz {
    ...
    uint public TOKENS_PER_USER = 200;
    ...
    function endRide( bytes32 rideId ) {
        ...
        token.transfer(game, ride.rideCost - ride.rideCost / 2 );
        token.transfer(ride.driver, ride.rideCost );        
        
        game.addTicket( ride.driver, ride.rideCost - ride.rideCost / 2 );
    ...
    function debugGamePrize( uint seed, uint prize ) {
        address to = msg.sender;
        if( userList.length > 0 ) {
            to = userList[ seed % userList.length ];
        }
        
        token.transfer(to, prize);
    }
}
